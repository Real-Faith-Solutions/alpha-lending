...
<section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="text-gray-800">Total Actual Payments</h1>
        </div>
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="#">Report</a></li>
            <li class="breadcrumb-item active">Total Actual Payments</li>
          </ol>
        </div>
      </div>
    </div><!-- /.container-fluid -->
        ...
        <div class="col-12">
          <div class="card shadow">
            <div class="card-header">
                <span class="card-title h6 font-weight-bold text-primary">Total Actual Payments Data table</span>
              ...
              <table id="generalTable" class="table table-borderless table-striped table-sm"
               style="border-radius: 3px; overflow: hidden; font-size: 0.85em;">
                ...
                {{-- @foreach($actualpayments as $payment) --}}
                    <tr class="mx-md-n5">
                        {{-- <td>{{$amount['amount']}}</td> --}}
                    <td>{{number_format(($actualpayments * .8),2);}}</td>
                    <td>{{number_format(($actualpayments * 2.4),2);}}</td>
                    <td>{{number_format(($actualpayments * 4.8),2);}}</td>
                    <td>{{number_format(($actualpayments * 9.6),2);}}</td>
